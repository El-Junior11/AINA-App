...
const Navbar=({darkMode,setDarkMode})=>{
  ...
  return <header className="h-16 flex items-center justify-between px-4 sm:px-6 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 sticky top-0 z-50 transition-colors font-body">
    ...
    <div className="flex items-center gap-1 sm:gap-1.5">
      <div className="relative group w-[24px] h-[24px] rounded-full border-2 border-slate-300 dark:border-slate-600 mr-1.5 hidden sm:flex items-center justify-center bg-slate-50 dark:bg-slate-800 shadow-inner">
      ...
      <button onClick={()=>setDarkMode(!darkMode)} className="relative group p-2.5 hover:bg-gradient-to-br hover:from-emerald-50 hover:to-sky-50 dark:hover:bg-slate-800 dark:hover:from-slate-800 dark:hover:to-slate-800 rounded-2xl text-slate-600 dark:text-white transition-all">
        {darkMode?<Sun size={19} className="text-amber-400"/>:<Moon size={19}/>}
        <Tooltip>{darkMode?"Activer le mode clair":"Activer le mode sombre"}</Tooltip>
      </button>
      <Link to="/notifications" className="relative group p-2.5 hover:bg-gradient-to-br hover:from-emerald-50 hover:to-sky-50 dark:hover:bg-slate-800 dark:hover:from-slate-800 dark:hover:to-slate-800 rounded-2xl text-slate-600 dark:text-white transition-all">
        <HelpCircle size={19} className="group-hover:scale-105 transition-transform"/>
        <Tooltip>Aide</Tooltip>
      </Link>
      <Link to="/settings" className="relative group p-2.5 hover:bg-gradient-to-br hover:from-emerald-50 hover:to-sky-50 dark:hover:bg-slate-800 dark:hover:from-slate-800 dark:hover:to-slate-800 rounded-2xl text-slate-600 dark:text-white transition-all">
        <Settings size={19} className="group-hover:rotate-90 transition-transform duration-300"/>
        <Tooltip>Paramètres du compte</Tooltip>
      </Link>
      {isAdmin&&<Link to="/admin/approvals" className="relative group p-2.5 hover:bg-sky-50 dark:hover:bg-slate-800 rounded-2xl text-slate-600 dark:text-white transition-all">
        ...
        <Tooltip>Approbation des inscriptions</Tooltip>
      </Link>}
