...
const Dashboard = () => {
  ...
  if (loading || !stats) return (
    <div className="h-screen flex flex-col items-center justify-center bg-white dark:bg-slate-950">
      ...
      <p className="font-semibold text-slate-500 dark:text-slate-400 text-sm tracking-wide">Chargement des données...</p>
    </div>
  );

  const kpiCards = [
    { label: 'Membres', val: stats.kpis.membres, icon: Users, col: 'text-sky-600 dark:text-sky-400', bg: 'bg-sky-50 dark:bg-sky-950/40', bar: 'from-sky-400 to-sky-600' },
    { label: 'Groupes GS', val: stats.kpis.groupes, icon: Home, col: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950/40', bar: 'from-emerald-400 to-emerald-600' },
    { label: 'Réseaux', val: stats.kpis.reseaux, icon: Network, col: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-950/40', bar: 'from-amber-400 to-amber-600' },
    { label: 'Responsables', val: stats.kpis.responsables, icon: ShieldCheck, col: 'text-violet-600 dark:text-violet-400', bg: 'bg-violet-50 dark:bg-violet-950/40', bar: 'from-violet-400 to-violet-600' },
    { label: 'Formations', val: stats.kpis.formations, icon: GraduationCap, col: 'text-rose-600 dark:text-rose-400', bg: 'bg-rose-50 dark:bg-rose-950/40', bar: 'from-rose-400 to-rose-600' },
  ];
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.evolutionGS}>
                ...
                <Tooltip 
                  ...
                  contentStyle={{borderRadius: '12px', border: '1px solid #e2e8f0', fontSize: '12px', background: '#ffffff', color: '#0f172a'}} 
                />
              ...
              <ResponsiveContainer width={220} height={220}>
                <PieChart>
                  <Pie 
                    ...
                    innerRadius={75} outerRadius={92} startAngle={180} endAngle={0} dataKey="value" stroke="none"
                  >
                    <Cell fill="#F59E0B" />
