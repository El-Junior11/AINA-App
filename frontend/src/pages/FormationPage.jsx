...
import {Search,Save,X,Loader2,FileText,Download,RefreshCw,Sheet,Pencil,GraduationCap,Eye,ClipboardCheck} from 'lucide-react';
...
const FormationPage=()=>{
  ...
  const getNomComplet=(membre)=>`${membre?.nom_membre||''} ${membre?.prenom_membre||''}`.trim();
  useEffect(()=>{fetchData()},[]);
  useEffect(()=>{
    const search=searchTerm.toLowerCase().trim();
    const results=data.filter(m=>{
      const nomComplet=getNomComplet(m).toLowerCase();
      const prenom=(m.prenom_membre||'').toLowerCase();
      const nom=(m.nom_membre||'').toLowerCase();
      const numero=(m.nummembre||'').toString().toLowerCase();
      return `${prenom} ${nom} ${numero}`.includes(search)||nomComplet.includes(search)||nom.includes(search)||prenom.includes(search)||numero.includes(search);
    });
  ...
  const fetchData=async()=>{
    try{
      ...
      const res=await axios.get('/formations',config);
      setData(res.data);
    }catch(err){
      console.error(err);
  ...
  const exportToPDF=async()=>{
    try{
      ...
      const getCircularImage=(url)=>new Promise((resolve,reject)=>{
        const img=new Image();
        img.onload=()=>{
          try{
      ...
      const tableRows=filteredData.map((m,i)=>[i+1,getNomComplet(m),m.formation?.gestionsimplifiee?'Oui':'Non',m.formation?.agroeco?'Oui':'Non',m.formation?.nutrition?'Oui':'Non',m.formation?.conservationproduit?'Oui':'Non',m.formation?.transformationproduit?'Oui':'Non',m.formation?.genre?'Oui':'Non',m.formation?.epracc?'Oui':'Non',m.formation?.autonomie?'Oui':'Non']);
  ...
  const exportToExcel=()=>{
    try{
      const excelData=filteredData.map((m,i)=>{
        const row={'N°':i+1,'Prénom':m.prenom_membre||'','Nom':m.nom_membre||'','Membre':getNomComplet(m),'ID':m.nummembre};
  ...
  const openEditModal=(m)=>{
    setSelectedMembre(m);
    setFormData({...initialForm,...(m.formation||{}),nummembre:m.nummembre});
    setShowModal(true);
  };
  ...
  const handleSubmit=async(e)=>{
    ...
    try{
      ...
      const res=await axios.post('/formations/save',dataToSave,config);
    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-950 transition-colors font-body overflow-hidden relative">
      ...
      <div className="px-4 md:px-6 py-5 flex items-center justify-between gap-4 flex-wrap">
        <div className="relative group flex-1 min-w-[240px] max-w-xl">
          ...
          <input type="text" placeholder="Rechercher un membre par prénom, nom ou ID..." className="w-full pl-11 pr-11 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/60 text-slate-900 dark:text-white focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none text-sm font-medium transition-all" value={searchTerm} onChange={e=>setSearchTerm(e.target.value)}/>
          {searchTerm&&<button type="button" onClick={()=>setSearchTerm('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-rose-500 transition-colors"><X size={17}/></button>}
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <button type="button" onClick={fetchData} disabled={loading} className="bg-sky-50 dark:bg-sky-950/30 text-sky-600 dark:text-sky-400 hover:bg-sky-100 dark:hover:bg-sky-950/50 px-3 py-2.5 rounded-xl transition-all font-bold text-xs border border-sky-100 dark:border-sky-900/40 shrink-0 flex items-center gap-2 disabled:opacity-60">
            <RefreshCw size={16} className={loading?'animate-spin':''}/>
            <span className="hidden sm:block">Actualiser</span>
          </button>
          <button type="button" onClick={()=>navigate('/formation-stats')} className="flex items-center gap-2 bg-sky-50 dark:bg-sky-950/30 text-sky-600 dark:text-sky-400 hover:bg-sky-100 dark:hover:bg-sky-950/50 px-3 py-2.5 rounded-xl transition-all font-bold text-xs border border-sky-100 dark:border-sky-900/40 shrink-0">
            <Eye size={16}/>
            <span className="hidden sm:block">Voir plus</span>
          </button>
          <div className="relative shrink-0">
            <button type="button" onClick={()=>setShowExportMenu(!showExportMenu)} className="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-2.5 rounded-xl transition-all flex items-center gap-2 text-xs font-bold shadow-sm hover:shadow-md">
              <Download size={16}/>
            ...
            {showExportMenu&&<div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-50 text-slate-900 dark:text-white overflow-hidden">
              <button type="button" onClick={exportToPDF} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800 text-sm font-medium border-b border-slate-100 dark:border-slate-800 transition-colors"><FileText size={17} className="text-rose-500"/>Document PDF</button>
              <button type="button" onClick={exportToExcel} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800 text-sm font-medium transition-colors"><Sheet size={17} className="text-emerald-600"/>Feuille Excel</button>
            </div>}
          </div>
        </div>
      </div>
                ...
                {loading?<tr><td colSpan={modules.length+4} className="p-12 text-center"><Loader2 size={28} className="animate-spin inline text-emerald-500"/></td></tr>:filteredData.map((m,index)=>(
                  <tr key={m.nummembre} className="h-[64px] hover:bg-emerald-50/40 dark:hover:bg-slate-800/50 transition-colors group">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-bold text-slate-900 dark:text-white capitalize truncate">{getNomComplet(m)}</p>
                          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mt-1">N° {m.nummembre||'-'}</p>
                        </div>
                      </div>
                    </td>
                    {modules.map(mod=><td key={mod.id} className="px-3 py-3 text-center"><span className={`px-2.5 py-1 rounded-lg text-xs font-bold ${m.formation?.[mod.id]?'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300':'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400'}`}>{m.formation?.[mod.id]?'Oui':'Non'}</span></td>)}
                    <td className="px-5 py-3">
                    ...
                    <td className="px-5 py-3 text-right w-24 sticky right-0 bg-white dark:bg-slate-900 group-hover:bg-emerald-50/40 dark:group-hover:bg-slate-800/50">
                      <button type="button" onClick={()=>openEditModal(m)} title="Modifier" className="p-2 text-sky-600 hover:bg-sky-50 dark:hover:bg-slate-800 rounded-lg transition-colors"><Pencil size={16}/></button>
                    </td>
                  </tr>
                ))}
        ...
        <div className="w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl rounded-2xl animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col max-h-[85vh]">
          <div className="p-5 bg-gradient-to-r from-emerald-500 to-sky-500 text-white flex justify-between items-center shrink-0">
            <div>
              ...
              <p className="text-sm font-medium text-white/80 mt-1 capitalize">{getNomComplet(selectedMembre)}</p>
            </div>
            <button type="button" onClick={()=>setShowModal(false)} className="text-white/80 hover:text-white p-2 rounded-xl hover:bg-white/10 transition-colors"><X size={20}/></button>
          </div>
          <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {modules.filter(m=>!m.isAuto).map(mod=><div key={mod.id} onClick={()=>handleCheckboxChange(mod.id,!formData[mod.id])} className={`flex items-center justify-between gap-2 p-3 rounded-xl cursor-pointer border transition-all ${formData[mod.id]?'border-emerald-500 bg-emerald-50/60 dark:bg-emerald-950/30':'border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40'}`}>
            ...
            <div className="pt-3">
              <button type="submit" disabled={saving} className="w-full bg-gradient-to-r from-emerald-500 to-sky-500 hover:opacity-90 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 text-sm transition-all shadow-sm hover:shadow-md disabled:opacity-60">
