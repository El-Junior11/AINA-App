...
const logAction = (req, res, next) => {
    ...
    res.send = async function (body) {
        ...
        if (['POST', 'PUT', 'DELETE'].includes(req.method) && res.statusCode >= 200 && res.statusCode < 300) {
            const tableName = req.originalUrl.split('/')[2];
            let details = {};
            ...
            if (req.method === 'PUT') {
                ...
                details = { message: `Suppression`, nom: getEntityLabel(tableName, deletedData) };
            } else {
                details = req.body;
            }
            ...
            const auditData = {
                ...
                target_id: (req.params.id && req.params.id !== 'undefined') ? req.params.id : null,
            ...
            try {
                ...
                if (global.io) {
                    global.io.emit('admin_alert', { ...auditData, details, User: { name: req.user?.name || "Administrateur" }, created_at: new Date() });
                }
